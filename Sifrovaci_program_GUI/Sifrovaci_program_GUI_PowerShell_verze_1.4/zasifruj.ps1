...
# odsud upraveno 16.1.2025, novy bezmaticovy spusob !!! <<<<<<<<<<<<<<<<<<<<<<<


$stream_reader=[System.IO.StreamReader]::new($nazev_klice)
...
for ( $aa12 = 0; $aa12 -le 125; $aa12++ ) { # 126 znaku hesla ( plati pro vsechny verze )
$poc_znaky = 0 # vzdy znovu definovat a nastavit na hodnotu 0 !!!
$rnd = Get-Random -Minimum 0 -Maximum 3843 # novy verze 1.3 max musi bejt 1-3843 !!!!

# TEST
#$rnd = 0 # test 0 = 00
#$rnd = 1 # test 1 = 01
#$rnd = 3842 # test 3843 = ZY
#$rnd = 3843 # test 3843 = ZZ
...
# dopocita pocet znaku do konce jednoho radku a preskoci je
...
for ( $bb22=0; $bb22 -le $d_pole_reference_na_heslo-1; $bb22++ ) {
#echo $pole_reference_na_heslo[$bb22]

$reference_na_heslo += [string] $pole_reference_na_heslo[$bb22]
if ( $bb22 -ne $d_pole_reference_na_heslo-1 ) { $reference_na_heslo += "," }
}
